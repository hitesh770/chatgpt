...
async def get_email_response(request: Request, sender_info: SenderInfo, receipient_info: RecipientInfo):
    response_sendor = requests.get(url=sender_info.website)
    response_reciepient = requests.get(url=receipient_info.website)
    if response_sendor.status_code == 200 and response_reciepient.status_code == 200:
        soup_sendor = BeautifulSoup(response_sendor.text, 'html.parser')
        soup_rec = BeautifulSoup(response_reciepient.text, 'html.parser')
        llm = ChatOpenAI(temperature=0.2, model=config.model,
                         openai_api_key=config.open_api_token)
        text_splitter = RecursiveCharacterTextSplitter(
            separators=["\n\n", "\n"], chunk_size=10000, chunk_overlap=500)
        docs = text_splitter.create_documents([soup_rec.get_text()])
        ...
        map_prompt = """
        I want you to act as a business-to-business cold sales email writer.
        I will provide you descriptions about the {product_or_service_offer}, about the target audience of specific business decision makers, and the sender's contact information for the signature line and {preferred_call_to_action}. 
        The fields available to merge in are {first_name}, {company_name}, and {city}. 
        Explain how the product or service can solve the {company_name_recipient} problems or help them achieve their goals. 
        Be specific and provide examples or case studies to back up your claims. Use a clear, concise subject line. 
        Personalize your email and use a casual and folksy friendly tone. Clearly state the value you can provide. 
        Keep it short and to the point. At the end of your email, make it clear what you want the {first_name_recipient} to do next. 
        This could be a phone call, a link to a calendar appointment, or visiting the {website}.:
        "{text}"
        SUMMARY:
        ...
        map_prompt_template = PromptTemplate(
            ...
            input_variables=["text", "product_or_service_offer", "preferred_call_to_action",
                             "first_name", "company_name", "city", "first_name_recipient", "website", "company_name_recipient"],
        ...
        output = summary_chain.run(input_documents=docs, product_or_service_offer=product_or_service_offer, preferred_call_to_action=preferred_call_to_action,
                                   first_name=first_name, company_name=company_name, city=city, first_name_recipient=first_name_recipient, website=website, company_name_recipient=company_name_recipient)
